...
def release_views():
    if request.method == 'GET':
        # 实现权限的验证(是否登录,是否为作者)
        # 判断用户是否在登录状态,如果登录了,则继续想下哦按段,否则去往登录也
        if 'uid' in session and 'uname' in session:
            # 继续判断是否为作者
            user = User.query.filter_by(id=session.get('uid')).first()
            if user.is_author != 1:
